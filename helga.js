...
try {
		...
		const katalog = (teks) => {
             res = helga.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Kazuko_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             helga.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = helga.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6282138919347-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© Kazuko_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database Kazuko```'
...
⚠️ Segala kebijakan dan ketentuan Kazuko di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 

Arigatou Gozaimasu! Untuk kalian user ramah dan Beberapa orang yg ikut membantu juga dalam project pembuatan Kazuko-
😖🙏

For GitHub : https://github.com/HelgaIlham
YouTube Helga : https://youtube.com/c/HelgaZexs妥
`
sendButLocation(from, `${menu}`, "*© Kazuko*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
case 'pornhub':
            ...
            reply (mess.wait)
              ...
              buttonsMessage = {footerText:'© Kazuko', imageMessage: imageMsg,
...
case 'cecan':
            ...
            reply (mess.wait)
              ...
              imageMsg = (await helga.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'© Kazuko', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT Kazukos`,buttons,headerType:4}
              prep = await helga.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              helga.relayWAMessage(prep)
              break                                           
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						helga.sendMessage(from, `© Kazuko`, text)
...
case 'ss':
             ...
             buttons = [{buttonId: `${prefix}menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await helga.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'© Kazuko', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT Kazuko!`,buttons,headerType:4}
              prep = await helga.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              helga.relayWAMessage(prep)
              break
                    case 'tiktokdl': 
...
┃💌 : YouTube : _*Kazuko's*_
┗━━⬣`
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nKazuko!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'rdp':
case 'vps':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
...
https://wa.me/6281398860278`
				sendButLocation(from, `${memek}`, `Subscribe Channel\nKazuko!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'jasrun':
case 'jasarun':
case 'kebutuhanbot':
  ...
  //── 「 Pembatas 」 ──//               
			...
			case 'runtime':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				sendButLocation(from, `${memek}`, `${tod}Subscribe Channel\nKazuko!!${tod}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
